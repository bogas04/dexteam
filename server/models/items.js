...
const FETCH_IN = "SELECT item_id,dish_family,restaurant_id,calorie,cross_items  from item_data where item_id in $itemsArray";
const FETCH_ALL = "SELECT item_id,dish_family,restaurant_id,calorie,cross_items from item_data limit $limit";
const ITEM_CALORIE = "SELECT item_id,calorie  from item_data order by calorie desc";
...
exports.fetchTheseItems = function(itemIdArray,  done) {
    ...
    db.get().query(sql_query, function (err, rows) {
        ...
        rows.forEach(function (item) {
            suggestion = []
            if (item.cross_items) {
                suggestion = SuggestionUtil.getSuggestedItems(JSON.parse(item.cross_items));
            }
...
function createCaloriesItemsMap(requestedCalorie, done) {
    ...
    if(Object.keys(calorieItemsMap).length === 0 && calorieItemsMap.constructor === Object) {
